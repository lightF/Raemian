	...
	var arr = new Array();
	//contextPath
	var contextPath = document.querySelector('#path2').value;
    ...
    function html5Upload() {	
		...
		for(var j=0, k=0; j < nImageInfoCnt; j++) {
			data.append('file'+j, htImageInfo['img'+j]);
    		
    	}
		jQuery3_4_1.ajax({
			url: document.querySelector('#path2').value +"/common/editor"
			, data: data
			//, method: "POST"
			,type: 'post'
			,data: data
			...
			,success: function(data) {
				for(var i = 0 ; i < data.result.length; i++){
					arr.push('<img src="'+data.result[i].path+'" alt="'+data.result[i].name+'" class="se-image-resource" width="'+data.result[i].width+'" height="'+data.result[i].height+'">')
				}
			
			}, complete : function(){
				opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [arr.join('')]);
				//테스트 중에는 팝업 닫기 비활성화
				closeWindow();
			}, error: function(xhr, textStatus, errorThrown) {
				alert('오류가 발생했습니다. 서버 환경을 점검해주세요.');
			}
		});
    	...
    	/*var tempFile;
    	var	sUploadURL;
    	sUploadURL= '/ISM/common/editor'; 	//upload URL
		//sUploadURL= '/smarteditor2/sample/photo_uploader/file_uploader_html5.php'; 	//upload URL
